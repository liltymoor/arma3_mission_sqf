...
//заносим поставленную технику в гараж
	...
	_Lifes = [];
	//Земля
	_dbLifes = parseSimpleArray ("extDB3" callExtension format ["0:PenaUpal:SELECT I_MRAP_03_hmg_F, I_MRAP_03_gmg_F, I_MBT_03_cannon_F, O_MBT_04_command_F, O_MBT_02_cannon_F, B_MBT_01_TUSK_F, I_APC_Wheeled_03_cannon_F, I_LT_01_AA_F, O_APC_Tracked_02_AA_F, B_APC_Tracked_01_AA_F FROM PlayerGarage WHERE UID='%1'",_uid]); 
	...
	for "_i" from 0 to count _dbLifes do {
		if (_dbLifes # _i != 0) then {
			switch (_i) do { 
				...
				case 6 : { _Lifes = _Lifes + ["I_APC_Wheeled_03_cannon_F"] }; //Gorgone
				case 7 : { _Lifes = _Lifes + ["I_LT_01_AA_F"] }; //Нюкс ПВ
				case 8 : { _Lifes = _Lifes + ["O_APC_Tracked_02_AA_F"] }; //Tigris
				case 9 : {  _Lifes = _Lifes + ["B_APC_Tracked_01_AA_F"] }; //Cheetah
			};
...
PENA_VEH_REWARD = {
        ...
        _killer = (_this # 1);
       switch (true) do {
      			 case (_unit isKindOf "I_MRAP_03_hmg_F") : {{[]remoteExec ["FREDDY_FNC_GETRANDOM_MNY_VEH", _x , false]} forEach crew vehicle _killer};
      			 case (_unit isKindOf "I_MRAP_03_gmg_F") : {{[]remoteExec ["FREDDY_FNC_GETRANDOM_MNY_VEH", _x , false]} forEach crew vehicle _killer};
                  ...
                  case (_unit isKindOf "I_LT_01_AA_F") : {{[]remoteExec ["FREDDY_FNC_GETRANDOM_MNY_VEH", _x , false]} forEach crew vehicle _killer};
                  ...
                  case (_unit isKindOf "B_Heli_Transport_01_camo_F") : {{[]remoteExec ["FREDDY_FNC_GETRANDOM_MNY_VEH", _x , false]} forEach crew vehicle _killer};

...
//Взаимодействие с выведенными из строя игроками
	if(_key == 219 && cursorObject isKindOf 'CAManBase' && player distance cursorObject <= 2.5 && lifeState cursorObject == "Incapacitated" && lifeState player != "Incapacitated" || _key == 220 && cursorObject isKindOf 'CAManBase' && player distance cursorObject <= 2.5 && lifeState cursorObject == "Incapacitated" && lifeState player != "Incapacitated") then {createDialog "InteractionPlayerMenu";}; //Winkey

//Взаимодействие с машинами
	if(_key == 219 && cursorObject isKindOf 'landvehicle' && player distance cursorObject <= 2.5 && (getDammage cursorObject) <= 0.99 && lifeState player != "Incapacitated"|| _key == 220 && cursorObject isKindOf 'landvehicle' && player distance cursorObject <= 2.5 && (getDammage cursorObject) <= 0.99 && lifeState player != "Incapacitated" || _key == 219 && cursorObject isKindOf 'AIR' && player distance cursorObject <= 2.5 && (getDammage cursorObject) <= 0.99 && lifeState player != "Incapacitated"|| _key == 220 && cursorObject isKindOf 'AIR' && player distance cursorObject <= 2.5 && (getDammage cursorObject) <= 0.99 && lifeState player != "Incapacitated") then {createDialog "VehInteraction";}; //Winkey
};
